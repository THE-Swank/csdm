...
int gmsgSpectator = 0; // SPECTATOR
int gmsgWaterSplash = 0;
int gmsgSmokeGrenade = 0;
int gmsgScopeToggle = 0;
int gmsgTeamMenu = 0;
int gmsgNewExplode = 0;
int gmsgDeleteAllDecals = 0;
int gmsgPlaySpecSnd = 0;
int gmsgTeamScores = 0;
int gmsgNewPrgBar = 0;
int gmsgKillPrgBar = 0;
int gmsgStartTimer = 0;
int gmsgKillTimer = 0;
...
void LinkUserMessages( void )
{
	...
	gmsgSpectator = REG_USER_MSG( "Spectator", 2 ); // SPECTATOR
	gmsgWaterSplash = REG_USER_MSG( "WaterSplash", 10 );
	gmsgSmokeGrenade = REG_USER_MSG( "Smoke", 6 );
	gmsgScopeToggle = REG_USER_MSG( "ScopeToggle", 1 );
	gmsgTeamMenu = REG_USER_MSG( "TeamMenu", 0 );
	gmsgNewExplode = REG_USER_MSG( "NewExplode", -1 );
	gmsgDeleteAllDecals = REG_USER_MSG( "DelDecals", 0 );
	gmsgPlaySpecSnd = REG_USER_MSG( "PlaySpecSnd", -1 );
	gmsgTeamScores = REG_USER_MSG( "TeamScores", -1 );
	gmsgNewPrgBar = REG_USER_MSG( "NewPrgBar", 1 );
	gmsgKillPrgBar = REG_USER_MSG( "KillPrgBar", 0 );
	gmsgStartTimer = REG_USER_MSG( "StartTimer", -1 );
	gmsgKillTimer = REG_USER_MSG( "KillTimer", 0 );
...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
	...
	case ACT_RELOAD:
		//ClientPrint( pev, HUD_PRINTCENTER, "reload" );
		if ( FBitSet( pev->flags, FL_DUCKING ) )	// crouching
...
{/*
	...
	if (pev->modelindex && (!m_fSequenceFinished) && (pev->deadflag == DEAD_DYING))
	{
		...
		if ( m_iRespawnFrames < 1024 )   // Animations should be no longer than this
			return;
	}
...
void CBasePlayer::PreThink(void)
{
	...
	if ( UTIL_PointContents(pev->origin) == CONTENT_BOMBPLANT1 )
        OnBombPlant1 = true;
    else
        OnBombPlant1 = false;

	if ( UTIL_PointContents(pev->origin) == CONTENT_BOMBPLANT2 )
        OnBombPlant2 = true;
    else
        OnBombPlant2 = false;
...
edict_t *EntSelectSpawnPoint( CBaseEntity *pPlayer )
{
	...
	if( g_pGameRules->IsTeamplay() && !g_pGameRules->IsHeadQuarters() )
	{
...
// dont let uninitialized value here hurt the player
	...
	/*
	if( !strncmp( m_szTeamName, "Terroristst", 6 ) )
		pev->team = 1;
	else
		pev->team = 2;
...
void CBasePlayer::DropPlayerItem ( char *pszItemName )
{
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		if ( pWeapon )
		{
			...
			pWeaponBox->pev->velocity = gpGlobals->v_forward * 200 + gpGlobals->v_up * 100;
