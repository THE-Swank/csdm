...
void CHalfLifeMultiplay :: UpdateGameMode( CBasePlayer *pPlayer )
{
	...
	WRITE_BYTE( gamemode.value );  // game mode none
	MESSAGE_END();
}
...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
	...
	if( RANDOM_LONG( 1, 2 ) == 1 )
		CLIENT_COMMAND( pl->edict(), "model Terrorists\n" );
	else
		CLIENT_COMMAND( pl->edict(), "model Counter-Terrorists\n" );
}
...
{/*
	...
	WRITE_SHORT( (pVictim->pev->iuser1 != 0) );
	...
	if( peKiller )
	{
		if( peKiller != pVictim && PlayerRelationship( pVictim, peKiller ) == GR_NOTTEAMMATE )
			peKiller->pev->frags++;
		else
			peKiller->pev->frags--;
		...
		WRITE_SHORT( (peKiller->pev->iuser1 != 0) );
		WRITE_SHORT( peKiller->pev->team );
		MESSAGE_END();
	}
}
...
void CHalfLifeMultiplay::DeathNotice( CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pevInflictor )
{
	...
	if( pVictim->m_LastHitGroup == 1 && strncmp( killer_weapon_name, "grenade", 7 ) )
		HeadShot = true;
	...
	if ( pVictim->pev != pKiller && ep && ep->Classify() == CLASS_PLAYER )
	{
		CBasePlayer *pPlayer = ( CBasePlayer * )ep;
		...
		if( !FirstBlood )
		{
			EMIT_SOUND(pPlayer->edict(), CHAN_ITEM, "qs/firstblood.wav", 1, ATTN_NONE);
			FirstBlood = true;
		}
			...
			EMIT_SOUND(pPlayer->edict(), CHAN_ITEM, "qs/hattrick.wav", 1, ATTN_NONE);

		else if(  pPlayer->Kills == 4 )
			EMIT_SOUND(pPlayer->edict(), CHAN_ITEM, "qs/godlike.wav", 1, ATTN_NONE);
